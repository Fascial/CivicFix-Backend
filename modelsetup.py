...
class Classifier:
    ...
    def department_selection(self, image_path, image_description=None):
        image_data = self.encode_image(image_path)

        category = self.categorize(image_path, image_description)

        prompt = f"""
            {self.information}
            Image Captions according to user: {image_description}
            The user description might sometimes be incomplete or faulty
            Issue Category (Can trust because it is generated by the model): {category}
        """
